...
describe("Paywall", () => {
  ...
  it("shows a swipe progress track instead of dots or arrows", async () => {
    const { getByTestId } = await renderWithProviders(<Paywall />);
    expect(getByTestId("paywall-progress-track")).toBeTruthy();
    expect(getByTestId("paywall-progress-fill")).toBeTruthy();
  });
});
